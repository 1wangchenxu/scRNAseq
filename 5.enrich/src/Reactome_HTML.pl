...
#@files = glob("$indir/*.reactome.xls");
my @files = `find $indir -name "*.reactome.xls"`;
...
CT
	...
	my @head = split /\t/, $head;
	my $t1 = "<table><caption>$pi Reactome Enrichment</caption><tr><th>#</th><th>$head[0]</th><th>$head[1]</th><th>$head[3]</th><th>$head[4]</th><th>$head[5]</th><th>$head[6]</th></tr>";
	my $t2 = "<table><caption>$pi Reactome Gene Details</caption><tr><th>#</th><th>$head[0]</th><th>$head[-1]</th></tr>";
	my $index = 0;
	while(<FA>)
	{
		chomp;
		...
		$t2 .= "<tr><td>$index</td><td><a href='$tmp[2]' title='click to view Reactome' target='_blank'>$tmp[0]</a></td><td style=\"text-align: left;\"><a name='gene$index'></a>$genes</td></tr>";
	}
	close FA;
	$t1 .= "</table>";
